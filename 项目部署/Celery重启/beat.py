...
celery_beat_pid = r'D:\Pros\TSDRM\celerybeat.pid'
if os.path.isfile(celery_beat_pid):
    os.remove(celery_beat_pid)
...
while True:
    ...
    if redis_ret and mysql_ret:
        try:
            subprocess.run(
                r'cmd /k "cd /d D:\Pros\TSDRM&&python manage.py celery -A TSDRM worker -l info')
        except:
