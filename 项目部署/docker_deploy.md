...
- `1.部署前的准备`  
    + 需要的容器：
        ...
        python_env:1.2
            >> python依赖包
            >> wkhtmltopdf(配合pdfkit模块使用的软件)
            >> cx_Oracle模块与oracle_client软件
            >> supervisor用以管理uwsgi与celery相关的进程
        redis:3.2.1
        mysql:5.6
        nginx:1.17.4
        ```
    + 需要准备的`文件目录`以及`项目相关文件`
        ```
        pro_tesu/
            >> Redis/
                >> conf/
                    ...
                    >> data/
            >> MySQL/
                >> conf.d/ (容器中mysql默认使用/etc/mysql/my.cnf配置文件，自定义时可以将*.cnf传至/etc/mysql/conf.d/文件夹下)
                    >> my.cnf
            >> TSDRM(所有项目在部署前都把名称改成这个)
            >> Nginx
                >> log/
                >> nginx.conf
            >> reload.sh (重启容器的脚本，需要加入主机开启启动脚本的位置)
        ```
        ```
        ...
        ```
        `redis.conf`，`my.conf`, `run.sh`, `reload.sh`以及`nginx.conf`文件的配置查看附录。
...
- `2.部署`  
    ...
    + 登录阿里云的docker Hub
        ```
        docker login --username=miaokela registry.cn-hangzhou.aliyuncs.com 
        ```
    ...
    + 拉取镜像
      ...
      docker pull registry.cn-hangzhou.aliyuncs.com/tesunet/python_env:1.2
      docker tag registry.cn-hangzhou.aliyuncs.com/tesunet/python_env:1.2 python_env:1.2
      ```
    + `运行容器`(避免主机已安装相关软件而占用端口，容器外端口：mysql-3307,redis-6378)
      ```
      cd /www/pro_tesu/
      docker run -p 3307:3306 --name tesudrm_mysql \
                        -v $PWD/MySQL/conf.d:/etc/mysql/conf.d \
					    -v $PWD/MySQL/logs:/logs \
					    -v $PWD/MySQL/data:/var/lib/mysql \
					    -e MYSQL_ROOT_PASSWORD=password \
					    -d mysql:5.6
      docker run -p 6378:6379 --name tesudrm_redis \
      ...
      docker run -v $PWD/Nginx/nginx.conf:/etc/nginx/nginx.conf \
            ...
            --link tmp_pro:django_server \
            --name tesudrm_nginx \
            -d -p 80:80 nginx   
      ```

- `3.容器说明`  
    + `cx_Oracle`模块与`oracle_client`软件的安装(`debian`系统)
      ```
      1) 先安装oracle_client: 
        需要文件：sdk-11.1.0.7.0-linux-x86_64.zip/basic-11.1.0.7.0-linux-x86_64.zip
        地址：https://www.oracle.com/database/technologies/instant-client/linux-x86-64-downloads.html

        >> 将压缩包解压至:/usr/local/文件夹下 
        >> 将sdk文件夹下的文件拷贝至/instantclient_11_1/
        >> 将so.11.1类似的文件创建软连接
            ln -s libocci.so.11.1 libocci.so
            ...
        >> 配置系统环境：/etc/profile或者~/.bashrc
            export ORACLE_HOME=/usr/local/instantclient_11_1
            export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$ORACLE_HOME
        >> source /etc/profile

      2) 安装cx_Oracle
          pip3 install cx_Oracle==5.2 -i https://mirrors.aliyun.com/pypi/simple
      ```
    ...
    + `supervisor的安装与使用`  
        ...
        # (忽略)如果在centos系统下，需要执行以下命令(Docker中debian系统不需要)
        ...
        # 修改以下内容
            # 将所有tmp路径修改成/var/run/，避免定时清空tmp目录导致进程启动失败
            # 修改权限
            chmod 777 /var/run/
            # 开启非守护进程，避免容器关闭
            nodaemon=true
...
- `4.设置开启自启`  
    ```
    # 修改Centos系统下的/etc/rc.d/rc.local文件添加一下内容：
    echo 'reload container...'
    source /wwww/pro_tesu/reload.sh
    ```
...
- `5.配置文件`
    - reload.sh
        ```
        #!/bin/bash
        reids_status=`docker inspect --format '{{.State.Running}}' tesudrm_redis`
        mysql_status=`docker inspect --format '{{.State.Running}}' tesudrm_mysql`
        pro_status=`docker inspect --format '{{.State.Running}}' tesudrm_pro`
        nginx_status=`docker inspect --format '{{.State.Running}}' tesudrm_nginx`

        # 删除pid文件
        cd $PWD/TSDRM/

        rm worker.pid beat.pid -f

        if [ "${reids_status}" != "true" ]; then
        docker start tesudrm_redis
        else
        docker restart tesudrm_redis
        fi

        if [ "${mysql_status}" != "true" ]; then
        docker start tesudrm_mysql
        else
        docker restart tesudrm_mysql
        fi
        sleep 2
        if [ "${pro_status}" != "true" ]; then
        docker start tesudrm_pro
        else
        docker restart tesudrm_pro
        fi
        sleep 2
        if [ "${nginx_status}" != "true" ]; then
        docker start tesudrm_nginx
        else
        docker restart tesudrm_nginx
        fi
        ```
